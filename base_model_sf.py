...
class BaseModel(nn.Module):
    ...
    def forward(self, v, q, labels, bias, v_mask, q_mask, loss_type = None):
        ...
        bias_pred=self.c_1(joint_repr.detach())

        bias_out=self.c_2(bias_pred)

        if labels is not None:
            if loss_type == 'q':
                loss = self.debias_loss_fn(None, bias_out, bias, labels)
                # loss = F.binary_cross_entropy_with_logits(bias_out, labels) * labels.size(1)

            elif loss_type == 'joint':
                ref_logits = torch.sigmoid(bias_out) + bias
                ...
                loss_q = F.binary_cross_entropy_with_logits(bias_out, labels) * labels.size(1)
                ref_logits = torch.sigmoid(bias_out)
